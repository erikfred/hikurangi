function [lamda, loadings, pcs, per] = ceof(data, varargin)

%   Complex (Hilbert) EOF:
% 
%   [lamda, loadings, pcs, per] = ceof(data);
% 
%   Outputs:
...
%   varargin(1) = nkp   = number of modes to output (default = 10);
%   varargin(2) = pd    = apply symmetric padding [t/f]
% 
...
%  data = (data - ones(200,1)*mean(data));
  ...
  if nargin == 3; nkp = varargin{1}; pd = varargin{2}; end;
  if nargin < 3; nkp = varargin{1}; pd = false; end;
  ...
  if pd
      data=padarray(data,ntim,npt,'symmetric');
  end
